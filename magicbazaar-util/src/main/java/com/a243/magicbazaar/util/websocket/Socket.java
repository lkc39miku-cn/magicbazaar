package com.a243.magicbazaar.util.websocket;

import com.a243.magicbazaar.dao.dto.SocketUserInfo;
...
public class Socket {
     ...
     * 保存所有连接上的用户的session
     */
    private static final Map<String, SocketUserInfo> userSessionMap = new ConcurrentHashMap<>();
    /**
     * 保存在线客服的session
     */
    private static final Map<String, SocketUserInfo> serverSessionMap = new ConcurrentHashMap<>();
    ...
    public void onOpen(Session session, @PathParam(value="role") Integer role) {
        ...
        //默认返回错误状态
        Map<String, String> resultMap = new HashMap<>(MAX_CONNECTION);
        ...
        if (role.equals(1)) {
            ...
            if (findFreeServer() != null){
                ...
                Map<String, String> result = new HashMap<>();
                //客服显示用户信息
                result.put("msg", "<div class='info'>正在为用户"+userInfo.getSessionId()+"服务！</div>");
                sendMsg(serverInfo.getSession(), JSON.toJSONString(result));
                result.put("msg", "<div class='info'>客服"+serverInfo.getSessionId()+"正在为您服务！</div>");
                sendMsg(userInfo.getSession(), JSON.toJSONString(result));
            } else {
                //告诉用户系统繁忙
            ...
            for (SocketUserInfo serverSocketInfo: serverSessionMap.values()) {
                ...
                if (serverSocketInfo.getTargetSessionId() != null && serverSocketInfo.getTargetSessionId().equals(session.getId())){
                    ...
                    if (lineUser != null){
                        ...
                        Map<String, String> result = new HashMap<>();
                        //客服显示用户信息
                        result.put("msg", "<div class='info-user'>正在为用户"+lineUser+"服务！</div>");
                        sendMsg(serverSocketInfo.getSession(), JSON.toJSONString(result));
                        //用户显示客户信息
                        result.put("msg", "<div class='info-help>>客服"+serverSocketInfo.getSessionId()+"正在为您服务！</div>");
    ...
    public void onMessage(String message, Session session) {
        ...
        if (serverInfo != null) {
            ...
            result.put("msg", "<div class='text-right box-shadow'>客服"+session.getId()+"："+message+"</div>");
            //将消息发送给用户
            ...
            result.put("msg", "<div class='text-left box-shadow'>"+message+"：用户"+session.getId()+"</div>");
            //将消息发送给客服
            //判断是否绑定了客服，如果有就发送消息
